...
@section('title', __('admin.create') . ' ' . __('admin.tags'))
@section('header', __('admin.create') . ' ' . __('admin.tags'))
@section('content')
<form action="{{ route('admin.tags.store') }}" method="POST" class="max-w-2xl space-y-6">
    @csrf
    <div class="bg-white dark:bg-gray-900 rounded-xl p-5 border border-gray-200 dark:border-gray-800 space-y-4">
        <div>
            ...
            <input type="text" name="name" id="name" value="{{ old('name') }}" required class="w-full px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 focus:ring-2 focus:ring-violet-500">
        </div>
        <div>
    ...
    <div class="flex gap-2"><button type="submit" class="py-2 px-6 bg-violet-600 hover:bg-violet-700 text-white text-sm font-medium rounded-lg">{{ __('admin.save') }}</button><a href="{{ route('admin.tags.index') }}" class="py-2 px-6 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 text-sm font-medium rounded-lg">{{ __('admin.cancel') }}</a></div>
</form>
@endsection
