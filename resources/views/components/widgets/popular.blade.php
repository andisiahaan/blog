    ...
    <div class="space-y-4">
        @foreach($posts as $post)
            <a href="{{ route('post.show', $post) }}" class="flex gap-3 group">
                ...
                @endif
                <div class="min-w-0">
                    <h4 class="text-sm font-medium text-gray-900 dark:text-white group-hover:text-violet-600 dark:group-hover:text-violet-400 line-clamp-2 transition-colors">{{ $post->title }}</h4>
                    <p class="text-xs text-gray-500 mt-1">{{ number_format($post->views) }} {{ __('common.views') }}</p>
                </div>
            </a>
        @endforeach
    </div>
